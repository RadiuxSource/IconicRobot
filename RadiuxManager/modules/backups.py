...
# from RadiuxManager.modules.sql import warns_sql as warnssql
import RadiuxManager.modules.sql.blacklist_sql as blacklistsql

# from RadiuxManager.modules.sql import cust_filters_sql as filtersql
# import RadiuxManager.modules.sql.welcome_sql as welcsql
import RadiuxManager.modules.sql.locks_sql as locksql
import RadiuxManager.modules.sql.notes_sql as sql

# from RadiuxManager.modules.rules import get_rules
import RadiuxManager.modules.sql.rules_sql as rulessql
from RadiuxManager import EVENT_LOGS,OWNER_ID, SUPPORT_CHAT, dispatcher
from RadiuxManager.__main__ import DATA_IMPORT
from RadiuxManager.modules.connection import connected
from RadiuxManager.modules.helper_funcs.alternate import typing_action
from RadiuxManager.modules.helper_funcs.chat_status import user_admin
from RadiuxManager.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open("RadiuxManager{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("RadiuxManager{}.backup".format(chat_id), "rb"),
        caption="➩ *sᴜᴄᴄᴇssғᴜʟʟʏ ᴇxᴘᴏʀᴛᴇᴅ ʙᴀᴄᴋᴜᴘ*\n➩ ᴄʜᴀᴛ ➛ `{}`\n➩ ᴄʜᴀᴛ ɪᴅ ➛ `{}`\n➩ ᴏɴ ➛ `{}`\n\n➩ ɴᴏᴛᴇ ➛ ᴛʜɪs `RadiuxManager-Backup` ᴡᴀs sᴘᴇᴄɪᴀʟʟʏ ᴍᴀᴅᴇ ғᴏʀ ɴᴏᴛᴇs 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("RadiuxManager{}.backup".format(chat_id))  # Cleaning file
