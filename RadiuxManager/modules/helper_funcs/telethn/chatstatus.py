...
from RadiuxManager import DRAGONS
from RadiuxManager.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Radiux_is_admin(chat_id: int):
    status = False
    Radiux = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Radiux.id == user.id:
            status = True
            break
    return status
