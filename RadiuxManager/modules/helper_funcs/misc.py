...
def paginate_modules(page_n: int, module_dict: Dict, prefix, chat=None) -> List:
    ...
    # Calculate the maximum number of pages based on the number of groups
    max_num_pages = ceil(len(pairs) / rows)

    # Calculate the modulo to determine the current page
    modulo_page = page_n % max_num_pages

    # Get the buttons for the current page
    page_buttons = pairs[modulo_page * rows: (modulo_page + 1) * rows]
