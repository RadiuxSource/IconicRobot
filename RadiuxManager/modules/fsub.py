...
from RadiuxManager import BOT_ID
from RadiuxManager import DRAGONS as DEVS
from RadiuxManager import OWNER_ID
from RadiuxManager import telethn as Radiux
from RadiuxManager.events import Radiuxinline
from RadiuxManager.events import register as Radiuxbot
from RadiuxManager.modules.no_sql import fsub_db as db


async def is_admin(chat_id, user_id):
    try:
        p = await Radiux(GetParticipantRequest(chat_id, user_id))
...
async def participant_check(channel, user_id):
    try:
        await Radiux(GetParticipantRequest(channel, int(user_id)))
        return True
    except UserNotParticipantError:
        return False
    except:
...
@Radiuxbot(pattern="^/(fsub|Fsub|forcesubscribe|Forcesub|forcesub|Forcesubscribe) ?(.*)")
...
✿ *ғᴏʀᴄᴇ ꜱᴜʙꜱᴄʀɪʙᴇ* ✿
   ...
   ➩ *ɪᴄᴏɴɪᴄ ᴄᴀɴ ᴍᴜᴛᴇ ᴍᴇᴍʙᴇʀꜱ ᴡʜᴏ ᴀʀᴇ ɴᴏᴛ ꜱᴜʙꜱᴄʀɪʙᴇᴅ ʏᴏᴜʀ ᴄʜᴀɴɴᴇʟ ᴜɴᴛɪʟ ᴛʜᴇʏ ꜱᴜʙꜱᴄʀɪʙᴇ*
   ...
   ➩ [ᴀᴅᴅ ᴍᴇ ɪɴ ʏᴏᴜʀ ɢʀᴏᴜᴘ ᴀꜱ ᴀᴅᴍɪɴ](https://t.me/Iconic_Robot?startgroup=new)
   ➩ [ᴀᴅᴅ ᴍᴇ ɪɴ your ᴄʜᴀɴɴᴇʟ ᴀꜱ ᴀᴅᴍɪɴ](https://t.me/Iconic_Robot?startgroup=new)
